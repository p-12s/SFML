...
unsigned eatAllCookiesInBounds(Field &field, const sf::FloatRect &bounds)
{
    ...
    for (size_t i = 0, n = field.width * field.height; i < n; i++)
    {
        ...
        if (cell.category != CellCategory::COOKIE && cell.category != CellCategory::SUPERCOOKIE)
        {
            continue;
        }
        ...
        if (cell.bounds.intersects(bounds, intersection) && (getArea(intersection) >= MIN_COOKIE_OVERLAP_AREA))
        {
            ++cookiesCount;
